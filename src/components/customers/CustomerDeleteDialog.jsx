...
export default function CustomerDeleteDialog({ open, onOpenChange, customer, onDeleted, isAdmin }) {
  ...
  const handleAdminDelete = async () => {
    setLoading(true);
    try {
      await base44.functions.invoke('customerDeleteManager', {
        action: 'delete', customer_id: customer.id,
      });
  ...
  const handleSubmitRequest = async () => {
    setLoading(true);
    try {
      const res = await base44.functions.invoke('customerDeleteManager', {
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-destructive">
            <Trash2 className="w-4 h-4" /> ลบข้อมูลลูกค้า
          </DialogTitle>
          <DialogDescription>{customer?.company_name} ({customer?.customer_code})</DialogDescription>
        </DialogHeader>
        ...
        {step === 'confirm' && (
          <div className="space-y-4 py-2">
            <div className="p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700 flex items-start gap-2">
              ...
              <div>
                <p className="font-medium">การลบลูกค้าจะไม่สามารถกู้คืนได้</p>
                <p className="text-xs mt-1">ข้อมูลงาน, billing, credential ที่เกี่ยวข้องจะยังอยู่ แต่ไม่มีลูกค้าอ้างอิง</p>
                {!isAdmin && <p className="text-xs mt-1 font-medium">คำขอลบจะถูกส่งไปให้ Admin อนุมัติก่อน</p>}
              </div>
            </div>
            {isAdmin ? (
        ...
        {step === 'reason' && (
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="เช่น ลูกค้ายกเลิกสัญญา, ข้อมูลซ้ำ..." rows={3} />
            </div>
            <div className="flex gap-2">
